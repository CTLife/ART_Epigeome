...
#              1. The table header of BED files must be removed. The suffix of input files must be ".bed".
#              2. The columns 1-4: chrom,  chromStart,  chromEnd,   name. 
#              3. The 1st to 4th columns is required and will be detected automatically.
## Example: 
#              Rscript  Functional_annotation.R    inputDir    outDir   organism
#              Rscript  Functional_annotation.R    Genomic_regions    Annotation_results    hg38
######################################################################################################################################################
...
RefGenome  = args_g[3];   
# inputDir = "Genomic_regions"
# outPath  = "Annotation_results" 
# RefGenome= "hg38"
...
suppressPackageStartupMessages( library(FusionExpressionPlot) ) 
...
sink( paste(outPath, "Parameters.log.txt", sep="/")   )
print( "my_txdb_g:"  )
print(  my_txdb_g  )
cat("\n\n\n\n\n") 
print( "my_orgdb_g: ")  
...
sink( paste(outPath, "All_the_input_files.txt", sep="/")   )
print(peakFiles)
...
MyPeaksAnno_OneGroup_1_g <- function(myPeak_t1, myPath_t1) { 
  myPeak_t1 = FusionExpressionPlot::grAddColumn(myPeak_t1, "V5", 1)
...
MyAnnotation_OneGroup_1_g <- function(myFile_t1, myPath_t1) {
  ...
  MyPeaksSignal_OneGroup_1_g(myPeak_t1 = myPeak_t1, myPath_t1 = paste(myPath_t1,  "2_GenomicRegions-on-TSSs-10kb",  sep="/"), halfSize = 10000)
  MyPeaksDistribution_OneGroup_1_g(myPeak_t1 = myPeak_t1, myPath_t1 = paste(myPath_t1,  "3_GenomicRegions-distribution-TSS5kb",  sep="/"), halfSize = 5000)
  MyPeaksDistribution_OneGroup_1_g(myPeak_t1 = myPeak_t1, myPath_t1 = paste(myPath_t1,  "4_GenomicRegions-distribution-TSS3kb",  sep="/"), halfSize = 3000)
  
  peakAnno <- annotatePeak( myPeak_t1, tssRegion=c(-3000, 3000),  TxDb=my_txdb_g, annoDb=my_orgdb_g  )  
  write.table(x=as.data.frame(peakAnno), file = paste(myPath_t1, "4_Genomic_regions_annotation_TSS3kb.txt", sep="/"),      
              append = FALSE, quote = FALSE, sep = "\t",   eol = "\n", na = "NA", dec = ".", row.names = FALSE, col.names = TRUE )
...
for(i in c(1:length(peakFiles)) ) { 
  myTempFunction3 <- function() {     
     MyAnnotation_OneGroup_1_g( myFile_t1 = peakFiles[i],  myPath_t1 = paste(outPath, "/File.", i, "._", peakFiles_onlyName[i],  sep="") )      
  }
  tryCatch(
    myTempFunction3(),
